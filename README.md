...
### 1. 투자 성향 테스트 (ML 기반)
- ML 기반 모델을 활용한 투자 성향 테스트 및 맞춤 투자 카테고리 추천
- 투자 성향 테스트 결과 이메일로 받아볼 수 있는 기능 제공
...
│   ├── keepit/                 # Django 설정
│   │   └── settings.py
│   ├── recommendations/        # 투자 성향 테스트 ML
│   ├── news/                   # 금융 뉴스
│   ├── products/               # 금융 상품(예적금, 주식 등)
│   ├── regions/                # 지역/은행 위치 검색
│   ├── scripts/                # 데이터 로딩, 크롤링 등 스크립트
│   ├── templates/              # 투자 성향 테스트 결과 이메일 템플릿
│   │   └── email/
│   ├── tests/                  # 투자 성향 테스트, 추천
│   └── users/                  # 회원 관리
│
