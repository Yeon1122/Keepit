...
### 7. 챗봇 상담
- **OpenAI GPT-3.5-turbo 기반** 실시간 금융 상담
  - 🤖 **생성형 AI 활용**: OpenAI API를 통한 자연어 대화
  - 📋 **2가지 전문 모드**: 웹사이트 도움말 / 금융 상품 상담
  - 💬 **대화 히스토리 관리**: 최근 5개 메시지 기억으로 맥락적 대화
  - 🎯 **전문화된 시스템 프롬프트**: 각 모드별 전문성 부여
- 홈페이지·금융상품 문의
- 대화 세션 관리
...
프로젝트의 데이터베이스 구조와 테이블 간의 관계를 나타낸 ERD입니다.

<div align="center">

![Keepit ERD](./docs/Keepit_ERD.png)
...
- **AI 챗봇**: **OpenAI GPT-3.5-turbo 기반** 실시간 금융 상담
  - **생성형 AI 활용**: OpenAI API를 통한 자연어 대화
  - **2가지 전문 모드**: 웹사이트 도움말 / 금융 상품 상담
  - **대화 히스토리 관리**: 최근 5개 메시지 기억으로 맥락적 대화
  - **전문화된 시스템 프롬프트**: 각 모드별 전문성 부여
...
#### 금융 상품 추천 알고리즘
Keepit의 핵심 기능인 개인화된 상품 추천은 다음과 같은 2단계 알고리즘으로 구현됩니다:

**1단계: 투자 성향 분석**
- 사용자의 투자 성향 테스트 결과를 기반으로 한 **머신러닝 분류 모델**
- 위험 선호도, 투자 목적, 투자 기간 등을 종합적으로 분석
- 3가지 투자 성향 유형으로 분류 (안정형, 중립형, 공격형)

**2단계: 상품 매칭 알고리즘**
- **XGBoost 기반 머신러닝 모델**을 통한 다중 라벨 분류
- 사용자 특성(나이, 성별, 소득, 자산, 위험성향 등) 10개 입력값 활용
- 5가지 상품 카테고리(예금, 적금, 주식, ETF, 현물)에 대한 추천 여부 결정
